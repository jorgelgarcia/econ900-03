...
\noindent Revisit matching on observed characteristics reinforcing the common-support assumption. Why? Because it should be clear that the objective is to obtain treatment-effect estimates for each value $\bm{x}_i = \bm{x}$ and then average across the support of $\bm{x}_i$.\\
...
\noindent \textbf{Conditional Independence Assumption.} The conditional-independence assumption is grants identification: 
\begin{align}
...
\begin{enumerate}
	\item Categorize $i \in \mathcal{I}$ into $J$ categories indexed by $\mathcal{J}$. The categories are based on the values of $\bm{x}_i$. Then, for the average difference between the subsample for which $D_i = 1$ and the subsample for which $D_i = 0$ for the category $j \in \mathcal{J}$: 
	\begin{align}
...
\end{align}
\noindent where $n_{dj}$ is the number of individuals in category $j \in \mathcal{J}$ for whim $D_i = d$. 
