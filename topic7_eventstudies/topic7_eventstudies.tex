...
\end{eqnarray}
\noindent where $\lambda_{t}$ and $\lambda_{e}$ are time and event-start units fixed effects, recovers a weighted average of the $\text{ATE} \left( e \right)_{t}$. If the panel is unbalanced in the time and event-start units space, the weights could have undesirable properties. The ``best''  $\text{ATE} \left( e \right)_{t}$ is a researcher's choice. 
